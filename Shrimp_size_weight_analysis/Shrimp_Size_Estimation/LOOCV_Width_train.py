...
output_dir = 'LOOCV_Width_predict'
...
X = data[['Image Estimated Width (mm)']].values
y = data['Actual Width (mm)'].values
...
for run in range(1, n_samples + 1):
    ...
    for x, y_true, y_p in zip(X_test.flatten(), y_test, y_pred):
        all_predictions.append({
            'Image Estimated Width (mm)': x,
            'Actual Width (mm)': y_true,
            'Predicted Width (mm)': y_p
        })
    ...
    ax1.set_xlabel('Image Estimated Width (mm)')
    ax1.set_ylabel('Actual Width (mm)')
    ...
    ax2.set_xlabel('Image Estimated Width (mm)')
    ...
    results = pd.DataFrame({
        'Image Estimated Width (mm)': X_test.flatten(),
        'Actual Width (mm)': y_test,
        'Predicted Width (mm)': y_pred,
        'Error (mm)': errors
    })
...
# Label mean and std rows properly
...
final_model.fit(X, y)
joblib.dump(final_model, os.path.join(final_model_dir, 'final_linear_model_width.pkl'))

# Save final model metrics
final_metrics = {'Slope': final_model.coef_[0], 'Intercept': final_model.intercept_}
pd.DataFrame([final_metrics]).to_excel(os.path.join(final_model_dir, 'final_model_metrics.xlsx'), index=False)
