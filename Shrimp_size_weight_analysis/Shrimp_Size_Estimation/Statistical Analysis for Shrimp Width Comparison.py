...
# Load LOOCV Width Prediction Results
df_path = os.path.join('LOOCV_Width_predict', 'summary', 'all_loocv_predictions.xlsx')
df = pd.read_excel(df_path)

# Extract actual and predicted values
actual = df['Actual Width (mm)'].values
predicted = df['Predicted Width (mm)'].values

# Calculate differences
differences = predicted - actual

print("=" * 70)
print("Statistical Analysis Results: Manual vs LOOCV Prediction (Width)")
print("=" * 70)
...
# 5. Create output folder and generate plots
output_folder = 'LOOCV_Width_statistical_analysis_results'
...
ax1.set_xlabel('Mean of Actual and Predicted Width (mm)')
...
ax2.set_xlabel('Actual Width (mm)')
ax2.set_ylabel('Predicted Width (mm)')
r_value = stats.pearsonr(actual, predicted)[0]
ax2.set_title(f'Actual vs Predicted Width (r = {r_value:.4f})')
ax2.legend(loc='best')
...
ax3.axvline(0, color='red', linestyle='--', linewidth=2.5)
ax3.axvline(mean_diff, color='blue', linestyle='-', linewidth=2.5)
...
ax6.axvline(np.mean(abs_errors), color='blue', linestyle='-', linewidth=2.5)
ax6.axvline(np.sqrt(np.mean(differences**2)), color='green', linestyle='--', linewidth=2.5)
...
if shapiro_p > 0.05:
    print("Conclusion: differences follow normal distribution ✓")
else:
    print("Conclusion: differences do not follow normal distribution")

print("\nAnalysis completed! All plots saved to folder:", output_folder)
