...
def run(data,
    ...
    for batch_i, (im, targets, paths, shapes) in enumerate(pbar):
        ...
        t3 = time_sync()
        out = non_max_suppression_obb(out, conf_thres, iou_thres, labels=lb, multi_label=True,
                                      agnostic=single_cls)  # list*(n, [xylsθ, conf, cls]) θ ∈ [-pi/2, pi/2)
        dt[2] += time_sync() - t3
    ...
    if len(stats) and stats[0].any():
        ...
        print(f'tp = {tp}, fp = {fp}, p = {p}, r = {r}, f1 = {f1}, map50 = {map50}')
...
def parse_opt():
    ...
    parser.add_argument('--data', type=str, default=r'data/my_shrimp_fft_data.yaml', help='dataset.yaml path')
    parser.add_argument('--weights', nargs='+', type=str, default=ROOT / r'runs/train/x_8_no_fft/weights/best.pt',
                        help='model.pt path(s)')
    parser.add_argument('--batch-size', type=int, default=35, help='batch size')
    parser.add_argument('--imgsz', '--img', '--img-size', type=int, default=840, help='inference size (pixels)')
