...
def run(weights=ROOT / 'yolov5s.pt',  # model.pt path(s)
    ...
    for path, im, im0s, vid_cap, s in dataset:
        ...
        # block = np.zeros([480, 300, 3], dtype=np.uint8)
        # block += 255
        # im0 = np.concatenate((im0s, block), 1)
        # sum_of_length = 0
        
        # Process predictions
        for i, det in enumerate(pred):  # per image
            pred_poly = rbox2poly(det[:, :5])  # (n, [x1 y1 x2 y2 x3 y3 x4 y4])
            # print(f'first:{pred_poly}')
            count = len(pred_poly)
            seen += 1
            ...
            # cv2.putText(im0, f"Count : ", (730, 35), 0, 0.75, (0, 0, 0), thickness=2, lineType=cv2.LINE_AA)
            # cv2.putText(im0, f"{count}", (820, 35), 0, 0.75, (0, 0, 0), thickness=2, lineType=cv2.LINE_AA)
            # cv2.putText(im0, f"Vein : ", (730, 70), 0, 0.75, (0, 0, 0), thickness=2, lineType=cv2.LINE_AA)
            # cv2.putText(im0, f"Not have vein : ", (730, 105), 0, 0.75, (0, 0, 0), thickness=2, lineType=cv2.LINE_AA)
            # cv2.putText(im0, f"Average length : ", (730, 140), 0, 0.75, (0, 0, 0), thickness=2, lineType=cv2.LINE_AA)
            ...
            if len(det):
                ...
                pred_poly = scale_polys(im.shape[2:], pred_poly, (480, 720))
                # print(f'sec:{pred_poly}')
                ...
                for *poly, conf, cls in reversed(det):
                    ...
                    if save_img or save_crop or view_img:  # Add poly to image
                        ...
                        # sum_of_length += annotator.test_poly_label(num_shown, poly, label, color=colors(c, True))
                        # annotator.test_poly_label(num_shown, poly, label=label, color=colors(c, True))
                        annotator.poly_label(poly, label, color=colors(c, True))
                        num_shown += 1
                    # if count:
                    # average_length = sum_of_length / count
                    # cv2.putText(im0, "{0:3f}".format(average_length), (930, 140), 0, 0.75, (0, 0, 0), thickness=2,
