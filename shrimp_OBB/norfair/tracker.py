...
class TrackedObject:
    ...
    def hit(self, detection: "Detection", period: int = 1):
        ...
        """
        self.weight = detection.weight
        self.length = detection.length
        self.width = detection.width
        self.frame = detection.frame
        
        # renew max
        #if self.max_weight is None or detection.weight > self.max_weight:
        #    self.max_weight = detection.weight
        if self.max_length is None or detection.length > self.max_length:
            self.max_length = detection.length
        if self.max_width is None and detection.width is not None:
            self.max_width = detection.width
        if detection.width is not None and detection.width > self.max_width:
            self.max_width = detection.width
        if self.max_weight is None and detection.weight is not None:
            self.max_weight = detection.weight
        if detection.weight is not None and detection.weight > self.max_weight:
            self.max_weight = detection.weight
            
        #if self.start_frame = detection.frame
        if self.start_frame is None:
            self.start_frame = detection.frame
        if self.end_frame is None or detection.frame > self.end_frame:
            self.end_frame = detection.frame
