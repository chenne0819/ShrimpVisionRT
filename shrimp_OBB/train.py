...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    save_dir, epochs, version, batch_size, weights, single_cls, evolve, data, cfg, resume, noval, nosave, workers, freeze = \
        Path(opt.save_dir), opt.epochs, opt.version, opt.batch_size, opt.weights, opt.single_cls, opt.evolve, opt.data, opt.cfg, \
            ...
            if opt.multi_scale and not opt.rect:
                ...
                if sf != 1:
                    ns = [math.ceil(x * sf / gs) * gs for x in
                          imgs.shape[2:]]  # new shape (stretched to gs-multiple) [h_new, w_new]
                    label_ratio = float(ns[0]) / imgs.shape[2]
                    imgs = nn.functional.interpolate(imgs, size=ns, mode='bilinear', align_corners=False)
                    targets[:,
                    2:6] *= label_ratio  # targets (tensor): (n_targets, [img_index clsid cx cy l s theta gaussian_θ_labels])
...
def parse_opt(known=False):
    ...
    parser.add_argument('--version', type=str, default='v5', help='set yolo version')
    parser.add_argument('--data', type=str, default=ROOT / 'data/bottom_shrimp.yaml', help='dataset.yaml path')
    parser.add_argument('--hyp', type=str, default=ROOT / 'data/hyps/obb/hyp.finetune_dota.yaml',
